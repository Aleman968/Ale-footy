...
def _summary_match_day_label(ts):
    try:
        return pd.to_datetime(ts, utc=True).tz_convert(APP_TIMEZONE).strftime("%d/%m/%Y")
    except Exception:
...
if view_mode == "Riepilogo giocate":
    ...
    summary_df["_day_sort"] = pd.to_datetime(summary_df["utcDate"], utc=True, errors="coerce").dt.date
    summary_df = summary_df.sort_values(
        ["_day_sort", "best_prob", "utcDate"],

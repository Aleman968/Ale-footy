...
st.markdown(
    ...
    .calendar-warn{color:#b45309;font-weight:850}
    .calendar-cup{color:#7c3aed;font-weight:850}
    .calendar-ok{color:#16854a;font-weight:850}
...
@st.cache_data(show_spinner=False, ttl=60*15)
def fetch_matches_date_window(start_date: str, end_date: str) -> pd.DataFrame:
    """Partite disponibili FootyStats in un intervallo arbitrario di date.
    Usa le competizioni accessibili all'account, incluse eventuali coppe presenti.
    """
    chosen_lookup = fetch_league_lookup(CHOSEN_LEAGUES_ONLY)
    if not chosen_lookup.get("leagues") and AUTO_RETRY_ALL_LEAGUES:
        chosen_lookup = fetch_league_lookup(False)
    ...
    try:
        d1 = dt.date.fromisoformat(str(start_date))
        d2 = dt.date.fromisoformat(str(end_date))
    except Exception:
        return pd.DataFrame()
    if d2 < d1:
        d1, d2 = d2, d1
    ...
    cur = d1
    while cur <= d2:
        data = api_get("/todays-matches", {"date": cur.isoformat(), "timezone": APP_TIMEZONE})
    ...
    if not df.empty:
        df["utcDate"] = pd.to_datetime(df["utcDate"], errors="coerce", utc=True)
        df = df.drop_duplicates(subset=["match_id"], keep="first").sort_values("utcDate")
    return df

def _is_cup_competition(name: str) -> bool:
    s = normalize_text(name)
    cup_terms = [
        "champions", "europa league", "conference", "coppa", "cup", "copa",
        "pokal", "coupe", "taca", "taça", "supercoppa", "super cup", "supercup",
        "knockout", "playoff", "play-off"
    ]
    return any(term in s for term in cup_terms)

def _calendar_events_for_team(calendar_df: pd.DataFrame, team_id: int, match_id: int, target_ts, max_days: int = 7) -> list:
    if calendar_df is None or calendar_df.empty or not team_id:
        return []
    target = pd.to_datetime(target_ts, utc=True, errors="coerce")
    if pd.isna(target):
        return []
    team_df = calendar_df[(calendar_df["home_id"] == int(team_id)) | (calendar_df["away_id"] == int(team_id))].copy()
    if match_id:
        team_df = team_df[team_df["match_id"] != int(match_id)]
    if team_df.empty:
        return []

    events = []
    for _, r in team_df.iterrows():
        ts = pd.to_datetime(r.get("utcDate"), utc=True, errors="coerce")
        if pd.isna(ts):
            continue
        delta_hours = (ts - target).total_seconds() / 3600.0
        delta_days_abs = abs(delta_hours) / 24.0
        if delta_days_abs > float(max_days) + 0.25:
            continue
        is_home = safe_int(r.get("home_id"), 0) == int(team_id)
        opponent = str(r.get("away_name" if is_home else "home_name") or "-")
        league = str(r.get("league_name") or "Competizione")
        direction = "prima" if delta_hours < 0 else "dopo"
        days_int = max(0, int(round(delta_days_abs)))
        events.append({
            "ts": ts,
            "direction": direction,
            "days": days_int,
            ...
            "opponent": opponent,
            "league": league,
            "is_cup": _is_cup_competition(league),
            "is_tight": abs(delta_hours) <= 96.0,
        })
    return sorted(events, key=lambda x: x["ts"])

def _calendar_team_html(team_name: str, events: list) -> str:
    if not events:
        return f'<div class="calendar-team">{team_name}</div><div class="calendar-item calendar-ok">Nessun impegno ravvicinato rilevato nei ±7 giorni</div>'
    rows = [f'<div class="calendar-team">{team_name}</div>']
    for e in events:
        try:
            when = e["ts"].tz_convert(APP_TIMEZONE).strftime("%d/%m %H:%M")
        except Exception:
            when = "-"
        rel = f'{e["days"]} gg {e["direction"]}'
        if e["is_cup"]:
            cls = "calendar-cup"
            icon = "🏆"
            tag = "COPPA"
            ...
            cls = "calendar-warn"
            icon = "⚠️"
            tag = "TURNO RAVVICINATO"
        else:
            cls = "calendar-ok"
            icon = "🗓"
            tag = "CALENDARIO"
        rows.append(
            f'<div class="calendar-item {cls}">{icon} {tag} · {when} · {e["league"]} · vs {e["opponent"]} · {rel}</div>'
        )
    return "".join(rows)
...
if view_mode == "Riepilogo giocate":
    ...
    # Menu a tendina multiselezione campionati. Il filtro viene applicato PRIMA
    # dell'analisi, così i campionati deselezionati non rallentano lo scanner.
    available_summary_leagues = sorted(all_matches_df["league_name"].dropna().astype(str).unique().tolist())
    saved_leagues = st.session_state.get("_summary_leagues_saved")
    if saved_leagues is None:
        saved_leagues = list(available_summary_leagues)
    else:
        saved_leagues = [x for x in saved_leagues if x in available_summary_leagues]
        if not saved_leagues:
            saved_leagues = list(available_summary_leagues)

    if "summary_leagues" not in st.session_state:
        st.session_state["summary_leagues"] = list(saved_leagues)
    else:
        st.session_state["summary_leagues"] = [
            x for x in st.session_state["summary_leagues"] if x in available_summary_leagues
        ] or list(saved_leagues)

    def _persist_summary_leagues():
        st.session_state["_summary_leagues_saved"] = list(st.session_state.get("summary_leagues", []))

    # Menu compatto a discesa: da chiuso mostra solo "Campionati".
    # Aprendolo compare l'elenco con una spunta per ogni campionato.
    with st.expander("Campionati ▾", expanded=False):
        _bc1, _bc2 = st.columns(2)
        with _bc1:
            if st.button("Seleziona tutti", key="summary_leagues_all", use_container_width=True):
                st.session_state["summary_leagues"] = list(available_summary_leagues)
                for _i in range(len(available_summary_leagues)):
                    st.session_state[f"summary_league_check_{_i}"] = True
                _persist_summary_leagues()
                st.rerun()
        with _bc2:
            if st.button("Deseleziona tutti", key="summary_leagues_none", use_container_width=True):
                st.session_state["summary_leagues"] = []
                for _i in range(len(available_summary_leagues)):
                    st.session_state[f"summary_league_check_{_i}"] = False
                _persist_summary_leagues()
                st.rerun()

        # Checkbox vere: l'elenco resta nascosto finché il menu è chiuso.
        current_selected = set(st.session_state.get("summary_leagues", available_summary_leagues))
        for _idx, _league in enumerate(available_summary_leagues):
            _ckey = f"summary_league_check_{_idx}"
            _default_checked = _league in current_selected
            if _ckey not in st.session_state:
                st.session_state[_ckey] = _default_checked

            _checked = st.checkbox(_league, key=_ckey)
            if _checked:
                current_selected.add(_league)
            else:
                current_selected.discard(_league)

        selected_summary_leagues = [x for x in available_summary_leagues if x in current_selected]
        st.session_state["summary_leagues"] = list(selected_summary_leagues)
        st.session_state["_summary_leagues_saved"] = list(selected_summary_leagues)

        st.caption(
            f"{len(selected_summary_leagues)} selezionati su "
            f"{len(available_summary_leagues)} campionati disponibili."
        )

    if not selected_summary_leagues:
        st.warning("Seleziona almeno un campionato da analizzare.")
        st.stop()

    summary_base = all_matches_df[
        all_matches_df["league_name"].isin(selected_summary_leagues)
        & all_matches_df["status"].isin(["SCHEDULED", "TIMED", "IN_PLAY", "PAUSED"])
    ].copy()
        ...
        with st.container(border=True):
            ...
            with c3:
                ...
                if st.button("Dettaglio →", key=btn_key, use_container_width=True):
                    ...
                    st.session_state["_summary_leagues_saved"] = list(st.session_state.get("summary_leagues", st.session_state.get("_summary_leagues_saved", [])))
    ...
    with dashboard_slot:
        ...
        # Calendario ravvicinato / coppe: cerca tutte le competizioni FootyStats
        # disponibili all'account nei 7 giorni prima e dopo la partita.
        try:
            target_local = pd.to_datetime(sel.get("utcDate"), utc=True).tz_convert(APP_TIMEZONE)
            cal_start = (target_local.date() - dt.timedelta(days=7)).isoformat()
            cal_end = (target_local.date() + dt.timedelta(days=7)).isoformat()
            calendar_df = fetch_matches_date_window(cal_start, cal_end)
            home_calendar = _calendar_events_for_team(calendar_df, home_id, safe_int(sel.get("match_id"), 0), sel.get("utcDate"), 7)
            away_calendar = _calendar_events_for_team(calendar_df, away_id, safe_int(sel.get("match_id"), 0), sel.get("utcDate"), 7)
            calendar_html = (
                '<div class="calendar-card"><div class="calendar-title">Calendario / impegni ravvicinati</div>'
                + _calendar_team_html(home_name, home_calendar)
                + _calendar_team_html(away_name, away_calendar)
                + '</div>'
            )
            st.markdown(calendar_html, unsafe_allow_html=True)
        except Exception:
            st.caption("Calendario impegni ravvicinati non disponibile per questa partita.")
